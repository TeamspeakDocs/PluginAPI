...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) {
	...
	int i = 0;
	enum { CMD_NONE = 0, CMD_JOIN, CMD_COMMAND, CMD_SERVERINFO, CMD_CHANNELINFO, CMD_AVATAR, CMD_ENABLEMENU, CMD_SUBSCRIBE, CMD_UNSUBSCRIBE, CMD_SUBSCRIBEALL, CMD_UNSUBSCRIBEALL, CMD_CREATEBOOKMARK } cmd = CMD_NONE;
#ifdef _WIN32
	char* context = NULL;
#endif
	...
	while(s != NULL) {
		if(i == 0) {
            ...
            } else if (!strcmp(s, "createbookmark")) {
                cmd = CMD_CREATEBOOKMARK;
            }
		} else if(i == 1) {
			param1 = s;
		} else {
	...
	switch(cmd) {
        ...
        case CMD_CREATEBOOKMARK:  /* /test createbookmark */
        {
            unsigned int error = ts3Functions.createBookmark(
                NULL,                                /* bookmark uuid. Autogenerated if empty or NULL. (Deprecated, somewhat dangerous to use if the uuid has invalid syntax, better to let it be autogenerated) */
                "TeamSpeak Public Server",           /* Label. Can be empty or NULL to fill with address. */
                "voice.teamspeak.com",               /* Address. Empty or NULL is invalid. */
                NULL,                                /* Server password. Can be empty or NULL. */
                "My nickname",                       /* Client nickname. Can be empty or NULL to use the nickname from identity. */
                NULL,                                /* Channel. Can be empty or NULL. */
                NULL,                                /* Channel password. Can be empty or NULL. */
                NULL,                                /* Capture profile. Can be name or empty/NULL. */
                NULL,                                /* Playback profile. Can be name or empty/NULL. */
                NULL,                                /* Hotkey profile. Can be name or uuid (just use name...). Can be empty or NULL for Always default. */
                "default_male",                      /* Sound profile. Can be short name ("default_male)" or long name ("Default Sound Pack (Male)") of the soundpack. Short name is defined by soundpack folder name, long name by "name" entry in the settings.ini of the soundpack. Can be empty or NULL for Always default. */
                NULL,                                /* Identity name. Can be empty or NULL for default identity */
                NULL,                                /* Privilege key (deprecated). Unused, just pass NULL. */
                "hello"                              /* Phonetic nickname. Can be empty or NULL. */
            );
            if (error != ERROR_ok)
            {
                ts3Functions.logMessage("Error creating bookmark", LogLevel_INFO, "Plugin", serverConnectionHandlerID);
            }
			break;
        }
	}
